...
キーリピート速度とリピート開始までの待ち時間は、システム設定で選択できる最速値に設定され、すぐに反映されます。

Caps Lockキーは、接続中のキーボードごとにControlキーへ割り当てられ、すぐに反映されます。割り当てはmacOSのホスト固有設定へ保存されるため、再起動後も維持されます。新しいキーボードを追加した場合は、`apply.sh`をもう一度実行してください。
